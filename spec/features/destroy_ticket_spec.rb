...
describe 'ユーザがイベントの参加表明をキャンセルする' do
  ...
  context '未ログインのユーザがイベント詳細ページに遷移したとき' do
    before { visit app.url(:events, :show, not_participating_event.id) }
  ...
  context 'ログインユーザが、未参加のイベント詳細ページに遷移したとき' do
    before do
      login
      visit app.url(:events, :show, not_participating_event.id)
    end
  ...
  context 'ログインユーザが、参加表明済みのイベント詳細ページに遷移したとき' do
    before do
      login
      visit app.url(:events, :show, participating_event.id)
    end
